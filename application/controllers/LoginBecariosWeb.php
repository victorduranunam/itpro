...
// Depuración
echo '<pre>';
var_dump($this->login->respuesta);
echo '</pre>';
exit; // detiene la ejecución para ver el resultado



            if(! $this->login->respuesta->status){
                $this->session->set_flashdata('mensaje', $this->login->respuesta->mensaje);               
        ...
        public function teminar_sesion(){
            ...
            $this->session->set_flashdata('mensaje', 'SesiÃ³n finalizada de forma correcta');               
            $this->session->set_flashdata('tipo_mensaje', 'satisfactorio');   
            redirect( base_url().'login');
        }
