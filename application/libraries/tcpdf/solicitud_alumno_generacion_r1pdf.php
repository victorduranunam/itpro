...
class MYPDF extends TCPDF {
    ...
    public function generaSolicitudImpresa($datos_personales, $preguntas_opcionales_solicitud,$preguntas_abiertas_solicitud){
        ...
        $foto_default_asset = asset_url().'media/img/face_icon.png';


        if (!empty($datos_personales->foto) && file_exists(FCPATH . 'fotos/' . $datos_personales->foto)) {
            $foto_asset = asset_url().'fotos/'.$datos_personales->foto;
            $this->Image($foto_asset, 172, 30 , 22, '', '', '', 'T', false, 300, '', false, false, 0, false, false, false);
        } else {
            // usar imagen genérica
            $this->Image($foto_default_asset, 172, 30 , 22, '', '', '', 'T', false, 300, '', false, false, 0, false, false, false);
        }
